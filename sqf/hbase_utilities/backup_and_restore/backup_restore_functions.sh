...
###############################################################################
#parse_srvr_user_name -- parse user name running o server process like hbase or hdfs ...
###############################################################################
...
validate_environment_and_users()
{
  ...
  which_environment
  which_env=$?
  echo "evironment: $which_env"  | tee -a $log_file
  ...
  then
    ...
    echo "***[ERROR]: Did not find supported Hadoop distribution or $MY_SQRROT is not set"
    return 1
  fi
