...
snapshot_list_file=${tmp_dir}/tafodion_snapshot_list_${date_str}.txt
...
if [[ -z "${hbase_root_dir}" ]] 
then 
   echo "***[ERROR]: Not able to determine hbase root dir. Please make sure all settings and classpatha are correct"    | tee -a $log_file
   exit 1
fi
...
hbase_cmd="$(get_hadoop_cmd) fs -copyToLocal ${hdfs_backup_location}/tafodion_snapshot_list.txt ${snapshot_list_file} "
...
cat <<EOF > ${tmp_dir}/tmp_list_tgt_tafodion_tables.hbase
list 'TRAFODION\..*\..*'
exit
EOF
hbase_cmd="$(get_hbase_cmd) shell ${tmp_dir}/tmp_list_tgt_tafodion_tables.hbase"
echo "${hbase_cmd}" | tee -a $log_file
${hbase_cmd} | tee   ${tmp_dir}/tmp_tgt_tafodion_table_list.txt | tee -a $log_file
grep 'TRAFODION\..*\..*'  ${tmp_dir}/tmp_tgt_tafodion_table_list.txt  > /dev/null
if [ $? -eq 0 ]; then
