...
public class RMInterface extends TransactionalTable{
    ...
    public synchronized TransactionState registerTransaction(final long transactionID, final byte[] row) throws IOException {
        ...
        if (ts == null) {
            ...
            //System.out.println("Register - Implicit begintransaction id " + transactionID + " TS " + ts);
            LOG.trace("registerTransaction, created TransactionState " + ts);
            mapTransactionStates.put(transactionID, ts);
            register = true;
        }
        else {
            //System.out.println("Register - Found TS for id " + transactionID);
        }
        ...
        if (register) {
    	    ...
    	    if (ts.islocalTransaction() == false) {
    	      LOG.trace("RMInterface:registerTransaction global transaction registering region.");
    	      registerRegion(lv_port, lv_hostname, lv_byte_region_info);
    	    }
    	    else {
    	      LOG.trace("RMInterface:registerTransaction local transaction not registered.");
    	    }
