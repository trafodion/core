		      ...
		      builder.setRegionName(ByteString.copyFromUtf8(Bytes.toString(regionName))); //ByteString.copyFromUtf8(Bytes.toString(regionName)));
	            ...
	            try {
                    ...
                    if ((location.getRegionInfo().compareTo(lv_hri) != 0)) {
                        ...
                        LOG.trace("doCommitX -- lv_hri: " + lv_hri);
                        LOG.trace("doCommitX -- location.getRegionInfo(): " + location.getRegionInfo());                        
	            ...
	            for (CommitResponse cresponse : result.values())
	            {
	              if(cresponse.getHasException()) {
	            	LOG.trace("doCommitX coprocessor exception: " + cresponse.getException());
	            ...
	            throw new UnknownTransactionException(); 
                    ...
                    retry = true;
                    retryCount++;
                    LOG.trace("doCommitX -- setting retry, count: " + retryCount);
	    ...
	    try {			
	     ...
	     try {
	        ...
	        for (CommitRequestResponse cresponse : result.values())
	        {
	          ...
	          commitStatus = value;        
	          if(cresponse.getHasException()) {
	        	LOG.trace("doPrepareX coprocessor exception: " + cresponse.getException());
	            throw new Exception(cresponse.getException());
	          }
	        }
	        retry = false;
	    	...
	    	retry = true;
	    	retryCount++;
	    	LOG.trace("doPrepareX -- setting retry, count: " + retryCount);
		    if(retryCount == RETRY_ATTEMPTS){
    ...
    public Integer doAbortX(final byte[] regionName, final long transactionId) throws IOException{
	    ...
	    do {
	      ...
	      {        		    	  
              ...
              retry = true;
              retryCount++;
              LOG.trace("doAbortX -- setting retry, count: " + retryCount);
              if (retryCount == RETRY_ATTEMPTS){
