...
//implements TransactionalRegionInterface {
  ...
  private IOException convertThrowableToIOE(final Throwable t, final String msg) {
    ...
    public void delete(final long transactionId, final byte[] regionName, final Delete delete) throws IOException {
    	...
    	if(transactionId == 0L){
			...
			try {
				...
				this.abortAutocommit(transRegion, id);
			}finally{
	 ...
	 * @throws IOException
	 */
	private void abortAutocommit(TransactionalRegion transRegion, long transId) throws IOException{
		try {
			transRegion.abort(transId);
		} catch (IOException e) {
			try {
				transRegion.abort(transId);
			} catch (Exception e1) {
				LOG.error("checkAndDelete, autocommit is true, ABORT failed, transID:" + transId, e1);
				throw new IOException("Autocommit is true, ABORT failed ",e1);
			}
		} catch (Exception e2) {
			LOG.error("checkAndDelete, autocommit is true, ABORT failed, transID:" + transId, e2);
			throw new IOException("Autocommit is true, ABORT failed ",e2);
		}
	}
