...
>>>>>>> 5b5e8fd... Implement single operation transaction (aka DP2 transactions)
    	...
    	if(transactionId == 0l){
			...
			try {
				...
				try {
					...
					LOG.trace("delete, autocommit is true, transID:" + id, e1);
				}
				throw e;
			}finally{
		...
		if(transactionId == 0l){
			...
			try {
				...
				try {
					...
					LOG.trace("put, autocommit is true, transID:" + id, e1);
				}
				throw e;
			}finally{
				this.transIDSet.remove(id);
		...
		if (transactionId == 0l) {
			...
			try {
				...
				try {
					...
					LOG.trace("puts, autocommit is true, transID:" + id, e1);
				}
				throw e;
			} finally {
				this.transIDSet.remove(id);
		...
		if (transactionId == 0l) {
			...
			try {
				...
				try {
					...
					LOG.trace("deletes, autocommit is true, transID:" + id, e1);
				}
				throw e;
			} finally {
				this.transIDSet.remove(id);
		...
		if (transactionId == 0l) {
			...
			try {
				...
				try {
					...
					LOG.trace("checkAndPut, autocommit is true, transID:" + id, e1);
				}
				throw e;
			} finally {
				this.transIDSet.remove(id);
		...
		if (transactionId == 0l) {
			...
			try {
				...
				try {
					...
					LOG.trace("checkAndDelete, autocommit is true, transID:" + id, e1);
				}
				throw e;
			} finally {
				this.transIDSet.remove(id);
