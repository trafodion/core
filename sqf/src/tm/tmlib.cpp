...
{
   printf ("REGISTERREGION ENTRY: txid: %ld, startId: %ld \n" , transid, startid);
   ...
   if (gp_trans_thr == NULL){
      printf ("REGISTERREGION gp_trans_thr is null\n");
      gp_trans_thr = new TMLIB_ThreadTxn_Object();
      gp_trans_thr->set_startid(lv_startid);
   }
   ...
   printf ("REGISTERREGION lv_savedStartId is %ld.  Startid is %ld \n", (long) lv_savedStartId, startid);
   if (lv_savedStartId <= 0){ // -1 or 0
      printf ("REGISTERREGION setting lv_savedStartId to %ld. \n", startid);
