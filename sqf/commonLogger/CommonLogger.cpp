...
*/
  ...
  try
  {
    char initParams[1024];
    sprintf( initParams, "configPath=%s, logFolder=%s, fileSuffix=%s", configPath.data(), logFolder_.data(), fileSuffix );
    ...
    //LoggerPtr logger(Logger::getLogger("CommonLogger"));
    //LOG4CXX_INFO(logger, initParams);
...
bool CommonLogger::isCategoryInDebug(std::string &cat)
{
    ...
    //return (myLogger->getLevel() != log4cxx::Level::getOff() &&
    //         myLogger->getLevel()->toInt() >= log4cxx::Level::DEBUG_INT);
