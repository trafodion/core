...
/*	static SQLMXDesc[] NewDescArray(SQLItemDescList_def desc) {
		...
		for (index = 0; index < desc.list.length; index++) {
			...
			SQLMXDescArray[index] = new SQLMXDesc(SQLDesc.dataType, (short) SQLDesc.datetimeCode, SQLDesc.maxLen,
...
*/
	...
	void executeDirect(int queryTimeout, SQLMXStatement stmt) throws SQLException {
		...
		//gr = inStmt_.ExecuteGeneric(executeAPI, messageBuffer);
...
//			txId = Bytes.createIntBytes(0, false);
		...
		/*
		if (ic_.t2props_.t2Logger_.isLoggable(Level.FINEST) == true) {
			Object p[] = T2LoggingUtilities.makeParams(stmt_.connection_.t2props, sql, queryTimeout, pstmt);
			String temp = "Exiting prepare...";
			ic_.t2props_.t2Logger_.logp(Level.FINEST, "InterfaceStatement", "prepare", temp, p);
		}
		*/
	};
	...
	byte[] getUDRTransaction(boolean swapBytes) throws SQLException {
		...
		try {
			...
			/* ic_.t2props_.t2Logger_.logp(Level.FINEST, "InterfaceStatement", "getUDRTransaction",
...
//			txId = Bytes.createIntBytes(0, false); // 0 length, no data
		...
		/*
		if (stmt_.connection_.t2props.getDelayedErrorMode())
		{
		...
		*/
	    ...
	    /* if (stmt_.connection_.t2props.getDelayedErrorMode() && stmt_._lastCount < 1) {
