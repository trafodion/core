...
final class InterfaceNativeResultset extends InterfaceNativeConnect {
	...
	FetchReply Fetch(int sqlAsyncEnable, int queryTimeout, long stmtHandle, int stmtCharset, int maxRowCnt,
		...
		try {
			//getInputOutput().setTimeout(m_ic.t2props_.getNetworkTimeout());
			byte[] bArray = null;
			int totalBytesReturned;

			LogicalByteArray buffer = FetchMessage.marshal(m_dialogueId, sqlAsyncEnable, queryTimeout, stmtHandle,
					m_stmtLabel, stmtCharset, maxRowCnt, 0 // infinite row size
					, cursorName, cursorCharset, stmtOptions, this.m_ic);

			bArray = TrafT2NativeInterface.processRequest(TRANSPORT.SRVR_API_SQLFETCH, stmtHandle, buffer.getBuffer());
	...
	CloseReply Close() throws SQLException {
		...
		try {
			...
			LogicalByteArray rbuffer = null;//getReadBuffer(TRANSPORT.SRVR_API_SQLFREESTMT, wbuffer);

			CloseReply cr = new CloseReply(rbuffer, null, /*m_ncsAddress.getIPorName(),*/ m_ic);
	...
	//public native byte[] processRequest(short executeAPI, long stmtId, byte[] buffer);
}
