...
class InterfaceResultSet {
		...
		case SQLTYPECODE_DATETIME:
			...
			case SQLDTCODE_TIMESTAMP:
				if (!useOldDateFormat) // new date format, if not SQLCatalogs
				{
					...
					length = desc.maxLen_;
					//Timestamp time = Timestamp.valueOf(new String(Bytes.read_chars(values, noNullValue, length)));
					retObj = Timestamp.valueOf(new String(Bytes.read_chars(values, noNullValue, length)));
					//retObj = time.toString();
				}
				break;
	...
	void close() throws SQLException {
		...
		if (rs_ != null && rs_.stmt_ != null && rs_.stmt_.ist_ != null
			...
			case odbc_SQLSvc_Close_exc_.odbc_SQLSvc_Close_SQLError_exn_:
				/*if (ic_.t2t2props.t2Logger_.isLoggable(Level.FINEST) == true) {
					Object p[] = T2LoggingUtilities.makeParams(rs_.connection_.t2props);
					String temp = "odbc_SQLSvc_Close_SQLError_exn_";
					rs_.connection_.t2props.t2Logger_.logp(Level.FINEST, "InterfaceResultSet", "close", temp, p);
				} */
...

	{
		...
		for (columnIndex = 0; columnIndex < columnCount; columnIndex++) {
			...
			/*
			if (conn.t2props.t2Logger_.isLoggable(Level.FINEST) == true) {
				Object p[] = T2LoggingUtilities.makeParams(conn.t2props, conn, desc, values);
				String temp = "Processing column = " + columnIndex + ":   noNullValueOffset = " + noNullValueOffset
						+ ":   nullValueOffset = " + nullValueOffset;
				conn.t2props.t2Logger_.logp(Level.FINEST, "InterfaceResultSet", "getExecute2Outputs", temp, p);
			}
			*/
