...
/* New initializer of SRVR_STMT_HANDL for rowsets purpose*/
SQLRETURN ALLOCSQLMXHDLS2(SRVR_STMT_HDL* pSrvrStmt)
{
...
SQLRETURN PREPARE2(SRVR_STMT_HDL* pSrvrStmt,bool isFromExecDirect)
{
    FUNCTION_ENTRY("PREPARE",
            ("pSrvrStmt=0x%08x",
             pSrvrStmt));
    ...
    if(srvrGlobal->bSpjEnableProxy)
    {
        ...
        {
            pSrvrStmt->SpjProxySyntaxString = new char[2000];
            if (pSrvrStmt->SpjProxySyntaxString == NULL)
            {
        ...
        if( (retcode == -8889) || requiredStringLen + 1 > pSrvrStmt->SpjProxySyntaxStringLen) // we need to null terminate the string
        {
            ...
            pSrvrStmt->SpjProxySyntaxString = new char[requiredStringLen + 1];
        ...
        Int32 IntLeadPrec, char *ColHeading)
{
    ...
    //fprintf(flog, "GetODBCValues: totalMemLen = %d\n", totalMemLen);
    //fclose(flog);
