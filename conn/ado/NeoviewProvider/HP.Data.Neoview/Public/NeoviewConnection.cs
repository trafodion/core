...
namespace HP.Data
{
    ...
    /// </summary>
    public sealed class HPDbConnection : DbConnection, IDbConnection
    {
        ...
        private HPDbMetaData _metaData;
        ...
        static HPDbConnection()
        {
            HPDbConnection._connPools = new Dictionary<string, ConnectionPool>();
        }

        /// <summary>
        /// Initializes a new instance of the HPDbConnection class.
        /// </summary>
        public HPDbConnection()
            : this(string.Empty)
        {
        }
        ...
        public HPDbConnection(string connectionString)
        {
            ...
            this.Commands = new List<HPDbCommand>();
            this._metaData = new HPDbMetaData(this);
        }
        ...
        public override string ConnectionString
        {
            ...
            set
            {
                ...
                {
                    string msg = HPDbResources.FormatMessage(HPDbMessage.InvalidConnectionState, this._state);
                    HPDbException.ThrowException(this, new InvalidOperationException(msg));
                }
        ...
        public override string Database
        {
            get
            {
                if (this._database == null)
                {
                    ...
                    using (HPDbCommand cmd = this.CreateCommand())
                    {
        ...
        { 
            get {
                ...
                {
                    HPDbException.ThrowException(this, new CommunicationsFailureException("Idle Timeout"));
                }
                return this._state;
            }
        }
        ...
        internal HPDbTransaction Transaction 
        { 
            get; 
            set; 
        }

        internal List<HPDbCommand> Commands // a list of the currently allocated commands
        { 
            get; 
            set; 
        } 
        ...
        internal HPDbEncoder Encoder
        {
            get { return this.Network.Encoder; }
        }

        /// <summary>
        /// Provides access to the default HPDbFactory instance.
        /// </summary>
        protected override DbProviderFactory DbProviderFactory
        {
            get
            {
                return HPDbFactory.Instance;
            }
        }
        ...
        public new void Dispose()
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this, TraceLevel.Public);
            }
        ...
        public new HPDbTransaction BeginTransaction()
        {
        ...
        /// <returns>An object representing the new transaction.</returns>
        public new HPDbTransaction BeginTransaction(IsolationLevel isolationLevel)
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this, TraceLevel.Public, isolationLevel);
            }
            ...
            this.Transaction = new HPDbTransaction(this, isolationLevel);
        ...
        public override void ChangeDatabase(string databaseName)
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this, TraceLevel.Public, databaseName);
            }
        ...
        internal void Close(bool forceClose)
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this, TraceLevel.Public, forceClose);
            }
            ...
            try
            {
                ...
                lock (this.dataAccessLock)
                {
                    ...
                    if (this.State != ConnectionState.Closed || forceClose)
                    {
                        ...
                        {
                            HPDbConnection._connPools[this.ConnectionStringBuilder.ConnectionString].AddPooledConnection(this);
        ...
        public new HPDbCommand CreateCommand()
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this, TraceLevel.Public);
            ...
            HPDbCommand cmd = new HPDbCommand(string.Empty, this, this.Transaction);
        ...
        /// <returns>The <code>HPDbCommand</code> object created</returns>
        public new HPDbCommand CreateCommand(string cmdLabel)
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this, TraceLevel.Public, cmdLabel);
            }

            HPDbCommand cmd = new HPDbCommand(string.Empty, this, this.Transaction, cmdLabel);
            this.Commands.Add(cmd);
        ...
        public override void Open()
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this, TraceLevel.Public, this.ConnectionStringBuilder.TraceableConnectionString);
            }
            ...
            ConnectionPool pool;
            Monitor.Enter(HPDbConnection._connPools);
            if (this.ConnectionStringBuilder.MaxPoolSize > 0)
            {
                if (!HPDbConnection._connPools.ContainsKey(connectionString))
                {
                    pool = new ConnectionPool(this.ConnectionStringBuilder);
                    HPDbConnection._connPools.Add(connectionString, pool);
                }
                else
                {
                    pool = HPDbConnection._connPools[connectionString];
                }

                HPDbConnection conn = pool.GetPooledConnection();
                if (conn != null)
                {
                    CopyProperties(conn);
                    initialized = true;
            ...
            Monitor.Exit(HPDbConnection._connPools);
        ...
        internal void SendInfoMessage(object sender, HPDbException e)
        {
            if (this.InfoMessage != null && e.Warnings.Count > 0)
            {
                this.InfoMessage(sender, new HPDbInfoMessageEventArgs(e.Warnings));
            }
        }
        ...
        internal bool RemoveCommand(HPDbCommand cmd)
        {
            if (HPDbTrace.IsInternalEnabled)
            {
                HPDbTrace.Trace(this, TraceLevel.Internal, cmd);
            }
            ...
            else if (this._state == ConnectionState.Open)
            {
                ...
                switch (reply.error)
                {
                    ...
                    default:
                        throw new HPDbException(HPDbMessage.InternalError, null, "FreeCommand", reply.error.ToString() + " " + reply.errorDetail + " " + reply.errorText);
                }
            }
        ...
        private GetObjRefReply GetObjRef()
        {
                ...
                try
                {
                    ...
                    switch (reply.error)
                    {
                        ...
                        case GetObjRefError.ASNoSrvrHdl:
                            throw new HPDbException(HPDbMessage.NoServerHandle);
                        case GetObjRefError.DSNotAvailable:
                            throw new HPDbException(HPDbMessage.DSNotAvailable);
                        case GetObjRefError.PortNotAvailable:
                            throw new HPDbException(HPDbMessage.PortNotAvailable);
                        ...
                        default:
                            throw new HPDbException(HPDbMessage.InternalError, "GetObjRef", reply.error.ToString() + " " + reply.errorDetail + " " + reply.errorText);
                    }
                }
                catch (Exception e)
                {
            ...
            if (!success) 
            {
                throw new HPDbException(HPDbMessage.TryAgain, retryEx!=null?retryEx.Message:null);
            }
            ...
            {
                this.SendInfoMessage(this, new HPDbException(HPDbMessage.ConnectedToDefaultDS, null, reply.datasource));
            }
            ...
            for (int i = 0; !success && i < retryCount; i++)
            {
                ...
                switch (reply.error)
                {
                    ...
                    case InitDialogueError.InvalidUser:
                        ...
                        {
                            e = new HPDbException(reply.errorDesc);
                            this.SendInfoMessage(this, e);
                            throw e;
                        }
                    ...
                    default:
                        throw new HPDbException(HPDbMessage.InternalError, null, "InitDiag", reply.error.ToString() + " " + reply.errorDetail + " " + reply.errorText);
                }
            }
        ...
        private void CopyProperties(HPDbConnection conn)
        {
