...
namespace HP.Data
{
    ...
    /// </summary>
    public sealed class HPDbCommand : DbCommand, IDbCommand, ICloneable
    {
        private HPDbConnection _conn;
        private HPDbTransaction _trans;
        ...
        private HPDbDataReader _reader;
        ...
        public HPDbCommand(string cmdText)
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(null, TraceLevel.Public, cmdText);
            }
        ...
        public HPDbCommand(string cmdText, HPDbConnection connection)
        ...
        public HPDbCommand(string cmdText, HPDbConnection connection, HPDbTransaction transaction)
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(connection, TraceLevel.Public, cmdText, transaction);
            }
        ...
        public HPDbCommand(string cmdText, HPDbConnection connection, HPDbTransaction transaction, string label)
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(connection, TraceLevel.Public, cmdText);
            }
        ...
        public override CommandType CommandType
        {
            ...
            set
            {
                ...
                {
                    throw new HPDbException(HPDbMessage.UnsupportedCommandType, null, value.ToString());
                }
        ...
        /// </remarks>
        public new HPDbTransaction Transaction
        {
            get
            {
                return this._trans;
        ...
        /// <summary>
        /// Gets or sets how command results are applied to the DataRow when used by the Update method of the HPDbDataAdapter. 
        /// </summary>
        ...
        IDbConnection IDbCommand.Connection
        {
            ...
            set
            {
                this.Connection = (HPDbConnection)value;
            }
        }
        ...
        IDbTransaction IDbCommand.Transaction
        {
            ...
            set
            {
                this.Transaction = (HPDbTransaction)value;
            }
        }

        internal HPDbDataReader DataReader
        {
            get
            {
                return this._reader;
        ...
        public new void Dispose()
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this.Connection, TraceLevel.Public);
            }
        ...
        /// <summary>
        /// Creates a new instance of a HPDbParameter object.
        /// </summary>
        /// <returns>A HPDbParameter object.</returns>
        public new HPDbParameter CreateParameter()
        {
            return new HPDbParameter();
        }

        /// <summary>
        /// Sends the CommandText to the Connection and builds a HPDbDataReader.
        /// </summary>
        /// <returns>A HPDbDataReader object.</returns>
        public new HPDbDataReader ExecuteReader()
        {
            return this.ExecuteReader(CommandBehavior.Default);
        }

        /// <summary>
        /// Sends the CommandText to the Connection, and builds a HPDbDataReader using one of the CommandBehavior values.
        /// </summary>
        /// <param name="behavior">One of the CommandBehavior values.</param>
        /// <returns>A HPDbDataReader object.</returns>
        public new HPDbDataReader ExecuteReader(CommandBehavior behavior)
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this._conn, TraceLevel.Public, behavior);
            }
            ...
            {
                this._reader = new HPDbDataReader(this);
            }
        ...
        public override int ExecuteNonQuery()
        {
            if (HPDbTrace.IsPublicEnabled)
            {
                HPDbTrace.Trace(this._conn, TraceLevel.Public,this._commandText);
            }
        ...
        public override object ExecuteScalar()
        {
            ...
            using (HPDbDataReader dr = this.ExecuteReader())
            {
                dr.FetchSize = 1;
        ...
        public override void Prepare()
        {
            ...
            if (!this._isPrepared)
            {
                ...
                {
                    ex = new HPDbException(this._prepareReply.errorList);
        ...
        /// Sends the CommandText to the Connection and builds a HPDbDataReader.
        /// </summary>
        /// <returns>HPDbDataReader</returns>
        ...
        {
            HPDbCommand cloneCommand = new HPDbCommand(this._commandText, this._conn, this._trans);
            ...
            foreach (HPDbParameter parameter in Parameters)
            {
        ...
        private void Execute()
        {
            ...
            if ((qt == QueryType.CallNoResults || qt == QueryType.CallWithResults) && reply.data != null && reply.data.Length > 0)
            {
                ...
                this._reader = new HPDbDataReader(this, null, paramReply);
                this._reader.Read();
            ...
            {
                this._reader = new HPDbDataReader(this, this._prepareReply, reply);
            }
            else if (reply.outputDesc.Length > 0)
            {
                this._reader = new HPDbDataReader(this, null, reply);
            }
            ...
            for (int i = 0; i < batchCount; i++)
            {
                for (int j = 0; j < paramCount; j++)
                {
                    ...
                    else
                    {
                        ...
                        if (desc.HPDbDataType == HPDbDbType.Varchar || 
                            desc.HPDbDataType == HPDbDbType.NVarchar)
                        {
                            dataLength += 2;
        ...
        {
            HPDbCommand command = (HPDbCommand)source;
            command.Cancel();
        }
        //batch support
