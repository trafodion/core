...
namespace HP.Data
{
    ...
    internal class HPDbNetwork : IDisposable
    {
        ...
        private HPDbConnection _connection;
        private HPDbEncoder _encoder;
        ...
        internal HPDbConnection Connection
        {
        ...
        internal HPDbNetwork(HPDbConnection conn)
        {
            if (HPDbTrace.IsInternalEnabled)
            {
                HPDbTrace.Trace(conn, TraceLevel.Internal);
            }
            ...
            this._encoder = new HPDbEncoder(this._byteOrder);
        }
        ...
        internal void OpenIO(string server, int port)
        {
            if (HPDbTrace.IsInternalEnabled)
            {
                HPDbTrace.Trace(this._connection, TraceLevel.Internal, server, port);
            }
            ...
            catch (Exception e)
            {
                ...
                HPDbException.ThrowException(this._connection, new CommunicationsFailureException(e));
            }
        ...
        internal void CloseIO()
        {
            if (HPDbTrace.IsInternalEnabled)
            {
                HPDbTrace.Trace(this._connection, TraceLevel.Internal);
            }
        ...
        internal void DoIO(INetworkMessage message, INetworkReply reply, OperationId id, DataStream fetchDs, BufferType fetchType, int IOtimeout)
        {
            if (HPDbTrace.IsInternalEnabled)
            {
                HPDbTrace.Trace(this._connection, TraceLevel.Internal, id);
            }
            ...
            if (this._isClosed)
            {
                string msg = HPDbResources.FormatMessage(HPDbMessage.InvalidConnectionState, this._connection.State);
                HPDbException.ThrowException(this._connection, new InvalidOperationException(msg));
            }
            ...
            try
            {
                ...
                if (HPDbTrace.IsNetworkEnabled)
                {
                    HPDbTrace.Trace(this._connection, TraceLevel.Network, id, "SEND", this.ToHexString(this._ds.Buffer, length));
                }
                // ***** BEGIN COMPRESSION *****
                ...
                if (HPDbTrace.IsNetworkEnabled)
                {
                    HPDbTrace.Trace(this._connection, TraceLevel.Network, id, "RECV", this.ToHexString(this._ds.Buffer, length));
                }
        ...
        private void Recv(int length, int IOTimeout)
        {
            ...
            if (totalRead < length)
            {
                HPDbException.ThrowException(this._connection, new CommunicationsFailureException("Socket unexpectedly closed"));
            }
        }
        /// <summary>
        /// Reset IdleTimer
        ...
        private void IdleTimerReset()
        {
            ...
            {
                HPDbException.ThrowException(this._connection, new CommunicationsFailureException("Idle Timeout"));
            }
            this._timer.Dispose();
        }
        /// <summary>
        ...
        {
            HPDbConnection connection = (HPDbConnection)source;
