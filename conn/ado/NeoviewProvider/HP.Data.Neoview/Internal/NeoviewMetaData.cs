...
namespace HP.Data
{
    ...
    /// </summary>
    internal class HPDbMetaData
    {
        private static string CatalogQuery = @"
select 
    rtrim(cat.cat_name) as CatalogName
from 
    HP_SYSTEM_CATALOG.SYSTEM_SCHEMA.CATSYS as cat
where 
...
from 
    HP_SYSTEM_CATALOG.SYSTEM_SCHEMA.CATSYS as cat inner join
    HP_SYSTEM_CATALOG.SYSTEM_SCHEMA.SCHEMATA as sch on cat.cat_uid = sch.cat_uid 
where 
...
from 
    HP_SYSTEM_CATALOG.SYSTEM_SCHEMA.CATSYS as cat inner join
    HP_SYSTEM_CATALOG.SYSTEM_SCHEMA.SCHEMATA as sch on cat.cat_uid = sch.cat_uid inner join 
...
from 
    ...
    HP_SYSTEM_CATALOG.SYSTEM_SCHEMA.SCHEMATA as sch on cat.cat_uid = sch.cat_uid inner join
    {0}.HP_DEFINITION_SCHEMA.OBJECTS as obj on sch.schema_uid = obj.schema_uid inner join
    {0}.HP_DEFINITION_SCHEMA.COLS as cols on obj.object_uid = cols.object_uid inner join
    {0}.HP_DEFINITION_SCHEMA.ACCESS_PATHS AS ap on obj.object_uid = ap.access_path_uid inner join
    {0}.HP_DEFINITION_SCHEMA.ACCESS_PATH_COLS AS apcols on 
...
for read uncommitted access";
        ...
        public HPDbMetaData(HPDbConnection conn)
        {
            _conn = conn;
            _dataset = null;
        ...
        private void Init()
        {
            ...
            if (_dataset == null)
            {
                ...
                _dataset.ReadXml(Assembly.GetExecutingAssembly().GetManifestResourceStream("HP.Data.Resources.Metadata.xml"), XmlReadMode.ReadSchema);
            }
        }
        ...
        public DataTable GetSchema(string collectionName, string[] restrictionValues)
        {
            if (HPDbTrace.IsInternalEnabled)
            {
                ...
                HPDbTrace.Trace(this._conn, TraceLevel.Internal, p);
            }
            ...
            switch (collectionName)
            {
                ...
                default:
                    HPDbException.ThrowException(this._conn, new InvalidOperationException("Invalid Collection Name: " + collectionName));
                    break;
            }
        ...
        private DataTable ExecuteQuery(string query)
        {
            if (HPDbTrace.IsInternalEnabled)
            {
                HPDbTrace.Trace(this._conn, TraceLevel.Internal, query);
            }

            DataTable dt;

            using (HPDbCommand cmd = this._conn.CreateCommand())
            {
                cmd.CommandText = query;
                using (HPDbDataAdapter da = new HPDbDataAdapter(cmd))
                {
                    dt = new DataTable();
                    da.Fill(dt);
                }
