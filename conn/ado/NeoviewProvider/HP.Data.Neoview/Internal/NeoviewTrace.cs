...
namespace HP.Data
{
    ...
    internal class HPDbTrace
    {
        ...
        public static TraceLevel Level 
        {
            get 
            {
                return HPDbTrace._level;
            }
            set
            {
                HPDbTrace._level = value;

                HPDbTrace.IsErrorEnabled = (HPDbTrace._level & TraceLevel.Error) > 0;
                HPDbTrace.IsWarningEnabled = (HPDbTrace._level & TraceLevel.Warning) > 0;
                HPDbTrace.IsPublicEnabled = (HPDbTrace._level & TraceLevel.Public) > 0;
                HPDbTrace.IsInternalEnabled = (HPDbTrace._level & TraceLevel.Internal) > 0;
                HPDbTrace.IsDetailEnabled = (HPDbTrace._level & TraceLevel.Detail) > 0;
                HPDbTrace.IsNetworkEnabled = (HPDbTrace._level & TraceLevel.Network) > 0;
            }
        }
        ...
        public static void Trace(HPDbConnection conn, TraceLevel level, params object[] p)
        {
            if ((HPDbTrace.Level & level) == 0)
            {
                return;
            }
            ...
            msg = string.Format(HPDbTrace.MessageFormat,
                DateTime.Now,
                level,
                connHashCode,
                connRemoteProcess,
            ...
            if (HPDbTrace._console)
            {
                System.Diagnostics.Trace.TraceInformation(msg);
            }

            if (HPDbTrace._writer != null)
            {
                _writer.Write(msg);
                _writer.Flush();
            }
