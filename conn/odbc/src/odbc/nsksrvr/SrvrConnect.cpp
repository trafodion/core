	...
	if(fd == -1){
		if ((fd = open(nameBuf, O_RDONLY)) == -1) {
			...
			if ((local_n = read(fd, dataBuf, sizeof dataBuf - 1)) < 0) {
				...
				sscanf(dataBuf, "%*ld %d %*ld %*ld %*ld %*ld %*ld", &memSize);
			}
		} else {
...
/*FILE *fp;
fp=fopen("/opt/home/xjian/1010","a+");
fprintf(fp,"In %s: memSize=%ld\n",sessionPhaseStr,memSize);
fclose(fp);*/	
	return memSize;
...
/*FILE *fp;
fp=fopen("/opt/home/xjian/1010","a+");
fprintf(fp,"In TerminateDialogue, server exit triggered\n");
fclose(fp);*/
		exitServerProcess();
	}
...
/*FILE *fp;
fp = fopen("/opt/home/xjian/1010","a+");
fprintf(fp,"In BreakDialogue,server exit triggered\n");
fclose(fp);*/
		exitServerProcess();
	}
