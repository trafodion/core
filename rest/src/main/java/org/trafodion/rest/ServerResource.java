...
public class ServerResource extends ResourceBase {
	 ...
	 * @throws IOException
	 */
	public ServerResource(String type) throws IOException {
		super();
		this.operation = type;
	}
	
	public String sqcheck() throws IOException {
	...
	public String pstack() throws IOException {
	    ScriptContext scriptContext = new ScriptContext();
	    scriptContext.setScriptName(Constants.SYS_SHELL_SCRIPT_NAME);
	    scriptContext.setCommand("sqpstack");
    ...
    public String dcs() throws IOException {
        ...
        try {
            ...
            JSONArray jsonRunning = new JSONArray();
            for (RunningServer aRunningServer: servers) {
                ...
                obj.put("REGISTERED",jsonRegistered);
                jsonRunning.put(obj);
            }

            jsonObject.put("RUNNING",jsonRunning);
	...
	public Response get(final @Context UriInfo uriInfo,@Context Request request) {
	    try {
	        ...
	        String result = null;
	        if(operation.equals("pstack"))
	            result = pstack();
	        else if(operation.equals("dcs"))
	            result = dcs();
	        else
	            result = sqcheck();
