      ...
      if (pkType->isEncodingNeeded())
        {
          ...
          else 
          {
            ...
            if ( rc == ex_expr::EXPR_OK ) {
              ...
              // will process prefix + result. Alignment length bytes at the 
              // start of decodeBuf can be disregarded. Give the previous 
              // assumption about size of varchar length indicator, alignment
              // bytes will be used by expression evaluator only if column
              // is of nullable type. For a description of how alignment is 
              // computed, please see ExpTupleDesc::sqlarkExplodedOffsets() in 
              // exp/exp_tuple_desc.cpp
              if (pkType->getSQLnullHdrSize() > 0)
                memcpy(&decodeBuf[resultOffset - pkType->getPrefixSize()], 
