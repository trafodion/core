...
#if 0
    // Maybe we don't need special case for PrivMgr schema?
    // If do, a better test would be to call 
    // bool isPrivMgrTable(const std::string &objectName);
    // See PrivMgrCommands.h
    // Also, if we special case PrivMgr schema here, we might need a change 
    // in GenericUpdate::bindNode to diallow altering the PrivMgr schema
    // table, as is Seabase MD tables - see code in that method that 
    // raises error 1391.
    if (strcasestr(tab->getTableName().getQualifiedNameAsString(),
