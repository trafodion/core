...
short ex_root_tcb::setupWarning(Lng32 retcode, const char * str,
...
Int32 ex_root_tcb::snapshotScanCleanup(ComDiagsArea* & diagsArea)
{
  ...
  const char* server = root_tdb().getHbaseServer();
  const char* zkPort = root_tdb().getZkPort();
  ...
  if (retcode!= 0)
  {
    if (!setupWarning(retcode, "ExpHbaseInterface::init", "", diagsArea))
  ...
  else
    {
      ...
      if (retcode!= 0)
        //continue even if setupWarning return -1
