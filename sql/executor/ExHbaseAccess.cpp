...
void ExHbaseAccessTcb::allocateDirectBufferForJNI()
{
...
short ExHbaseAccessTcb::createDirectRowwiseBuffer(char * inputRow)
{
  ...
  curPtr = inputRow;
  str_cpy_all((char*)&numEntries, curPtr, sizeof(short));
  curPtr += sizeof(short);
  ...
  for (Lng32 ij = 0; ij < numEntries; ij++)
  {
      ...
      curPtr += sizeof(short);
      colVal = curPtr;
      curPtr += ROUND2(colValLen);
      if (! nullable)
      {
         rowLen += (colNameLen + colValLen + 1 // for nullValChar
                     + (2 * sizeof(short)));  
         numCols++;
      }
  }
  
  allocateDirectBufferForJNI();
  BYTE *rowCurPtr = (BYTE *)row_.val;
  *(short *)rowCurPtr = bswap_16(numCols);
  rowCurPtr += sizeof(short);
  row_.len += sizeof(short);
  
  curPtr = inputRow; 
  curPtr += sizeof(short); // skip the numEntries
  for (Lng32 ij = 0; ij < numEntries; ij++)
  {
      colNameLen = *(short*)curPtr;
      curPtr += sizeof(short);
