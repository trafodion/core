  ...
  , exception_(FALSE)
{
  ...
  Lng32 fileNum = getGlobals()->castToExExeStmtGlobals()->getMyInstanceNumber();
   buildLoggingPath(((ExHdfsScanTdb &)hdfsScanTdb).getLoggingLocation(),
  ...
  while (!qparent_.down->isEmpty())
	  ...
	  if (rowWillBeSelected)
	  {
	    if (moveColsConvertExpr())
	    {
	      ...
	      if (evalRetCode == ex_expr::EXPR_ERROR)
	      {
	        if (hdfsScanTdb().continueOnError())
	        {
	          //may create a functuon with this repeting code???
                  if ( workAtp_->getDiagsArea())
                  {
                   ...
                   nextStep_ = step_;
                   step_ = HANDLE_EXCEPTION;
	          //logError(hdfsLoggingRow_, hdfsLoggingRowEnd_ - hdfsLoggingRow_ +1);
	          break;
	        }
	        step_ = HANDLE_ERROR_WITH_CLOSE;
	        break;
	...
	case HANDLE_EXCEPTION:
	{
	  ...
	  if (hdfsScanTdb().getMaxErrorRows() > 0)
	  {
	    ...
	    // if need to log counts -- add test
	    incrErrorCount( ehi_,exceptionCount, hdfsScanTdb().getErrCountTable(),hdfsScanTdb().getErrCountRowId());
	    if (exceptionCount >  hdfsScanTdb().getMaxErrorRows())
	    {
	      ...
	      *da << DgSqlCode(-EXE_MAX_ERROR_ROWS_EXCEEDED);
          ...
          if (hdfsScanTdb().getLogErrorRows())
          {
            ...
            memcpy (loggingErrorMsg_, hdfsLoggingRow_,loggingRowLen );

            charBuf *cBuf = NULL;

            if ( lastErrorCnd_ != NULL)
            {
              errorMsgLen = lastErrorCnd_->getMessageLength(); //wcslen(lastErrorCnd_->getMessageText())*2;
              const NAWcharBuf wBuf((NAWchar*)lastErrorCnd_->getMessageText(), errorMsgLen, getHeap());
              cBuf = unicodeToISO88591(wBuf, getHeap(), cBuf);
              errorMsgLen = cBuf -> getStrLen();
              memcpy (&loggingErrorMsg_[loggingRowLen], (char*)cBuf->data(),errorMsgLen );
              loggingErrorMsg_[loggingRowLen+errorMsgLen]='\n';
              errorMsgLen++;
            }
            else
            {
              errorMsgLen = strlen("[UNKNOWN EXCEPTION]");
              memcpy (&loggingErrorMsg_[loggingRowLen], (char*)"[UNKNOWN EXCEPTION]",errorMsgLen );
              loggingErrorMsg_[loggingRowLen+errorMsgLen]='\n';
              errorMsgLen++;
            }

            handleException(loggingErrorMsg_,
                loggingRowLen + errorMsgLen,
                ehi_,
                LoggingFileCreated_,
                loggingPath_);
          }
...
void ExHdfsScanTcb::incrErrorCount( ExpHbaseInterface * ehi,Int64 & totalExceptionCount,
                                    const char * tabName, const char * rowId )
{
  Lng32 retcode = ehi->initHBLC();
...
void ExHdfsScanTcb::buildLoggingPath(
                             ...
                             char * loggingPath)
{
  ...
  sprintf(loggingPath, "%s/ERR_%s/%s_%s_%d",
                  loggingLocation, logId, tableName, loggingFileName, instId); 
}

void ExHdfsScanTcb::handleException(char * data,
                             ULng32 bytesToWrite,
                             ...
                             char * loggingPath)
{
  ...
  if (data)
  {
    ...
    {
      retcode = ehi->hdfsCreateFile(loggingPath);
      LoggingFileCreated = TRUE;
    }
    retcode = 0;
    retcode = ehi->hdfsWrite(data, bytesToWrite);
  }
}
