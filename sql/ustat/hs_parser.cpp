                             ...
                             NABoolean& saltMatched)
{
  ...
  // If both _SALT_ and the last index column were matched and the index has more
  // columns than the group, then something in the middle was left out, and we
  // can not say the group matches the index.
  if (numGrpCols < numInxCols && saltMatched && lastInxColMatched)
    return FALSE;
  else
    return TRUE;
}
  ...
  while (group)
    {
      ...
      if (MatchesIndexPrefix(group, clusteringIndex, groupHasSalt))
        AddSaltedIndexPrefix(clusteringIndex, group, groupHasSalt);
