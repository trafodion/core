...
public:
  ...
  void setUseCif(NABoolean v)
   ...
   NABoolean continueOnError() { return (flags_ & CONTINUE_ON_ERROR) != 0; };
    ...
    void setLogErrorCount(NABoolean v)
    {(v ? flags_ |= LOG_ERROR_COUNT : flags_ &= ~LOG_ERROR_COUNT); };

    NABoolean getLogErrorCount() { return (flags_ & LOG_ERROR_COUNT) != 0; };
