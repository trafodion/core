...
void CharType::createStringLiteral(char * buf,
                                   ...
                                   CharInfo::CharSet targetCS) const
{
  ...
  NAString *result = new(h) NAString(getCharSetAsPrefix());
  ...
  *result += "'";
  ...
  switch (sourceCS)
    {
    case CharInfo::UTF8:
      *result += NAString((char *) buf, bufLen);
      break;
    ...
    case CharInfo::UNICODE:
      {
        ...
        int retCode = LocaleToUTF8(cnv_version1,
                                   ...
                                   &outputLength);
        if (retCode == 0)
          *result += NAString(tempBuf, outputLength);
        else
          *result += "<char conv. error>";
      }
      break;
  ...
  size_t firstQuotePos = result->first('\'');
  size_t otherQuotePos = firstQuotePos + 1;
  while ((otherQuotePos = result->first('\'', otherQuotePos)) != NA_NPOS)
    {
      result->replace(otherQuotePos, 1, "''");
      otherQuotePos += 2;
    }
...
// 2/19/98: checked for unicode. No need to add Unicode case.
  ...
  if (stringLiteral)
    createStringLiteral((char *) bufPtr, stringLiteral, h);
}
				       ...
				       NABoolean sqlmpKeyGen) const
{
  ...
  switch (getBytesPerChar())
  {
    case 4: // sames SQL_UTF8_CHAR_MAXSIZE
      ...
      fillWithMaxUTF8Chars(&((char *)bufPtr)[i],bufCnt-1,getPrecisionOrMaxNumChars());
      break;
    case 1:
      max_char = (char)getMaxSingleCharacterValue();
      for ( ;i < bufCnt; i++)
