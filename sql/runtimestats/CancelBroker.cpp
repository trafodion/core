...
#else
      const short *statsPhandle = rootOperStats ? 
                                  rootOperStats->getPhandle() :
                                  measStats->getPhandle();
#endif // NA_LINUX
      if (statsPhandle)
      {
...
#ifdef NA_LINUX
        // Cleanup of shared segment is made by this process, when the monitor
        // sends its ZSYS_VAL_SMSG_PROCDEATH message.  No need to delay here.
#else
        // Let NSK stop catcher run.
        ...
        if (pq->getCancelLogging())
        {
          ...
          char msg[80 + // the constant text
                 ComSqlId::MAX_QUERY_ID_LEN + 
                 40   // the process name (e.g., \NEODB2.$:6:1042:40143296) 
                  ];
