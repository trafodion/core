    ...
    pendingQueryMgr_(this, ssmpheap)
{
...
#ifndef NA_LINUX
  str_cpy_all (defineName, "=_MX_RTS_MERGE_INTERVAL ", 24);
  if ((error = getDefineNumericValue(defineName, &mergeInterval)) == 0)
...
#ifndef NA_LINUX
  str_cpy_all (defineName, "=_MX_RTS_MERGE_TIMEOUT  ", 24);
  if ((error = getDefineNumericValue(defineName, &statsTimeout)) == 0)
  {
    if (statsTimeout > 1)
...
#ifndef NA_LINUX 
  str_cpy_all (defineName, "=_MX_RTS_SQL_SOURCE_LEN ", 24);
  if ((error = getDefineNumericValue(defineName, &sqlSrcLen)) == 0)
  {
    if (sqlSrcLen < 0)
      storeSqlSrcLen_ = 0;
    else
...
#else
  short sscpProcHandle[10];
  (void)phandle.getmine((short *)&statsGlobals->ssmpProcHandle_);
#endif
  ...
  str_cpy_all (defineName, "=_MX_SSMP_FORCE_MERGE   ", 24);
...
ULng32 SsmpGlobals::allocateServers()
{
  ...
  if (sscpServerClass_ == NULL)
  {
#ifndef NA_LINUX
    if (ComRtIsNeoSystem())
      noOfSegs = ComRtGetNeoSegsInfo(segInfo, MAX_NO_OF_SEGMENTS, noOfSegs); 

    if (noOfSegs == 0)
      noOfSegs = ComRtGetNodeInfo(&segInfo[0]);
    ...
    {
      for (j = 0 ; j < segInfo[i].noOfCpus_ ; j++)
      {
        // cpu 0 status is in the high order bit
        if (segInfo[i].cpuStatus_ & (0x8000 >> j)) 
        {
          allocateServer(segInfo[i].segName_, (short)str_len(segInfo[i].segName_), (short)j);
        }
      }
    }
#else
...
IpcServer *SsmpGlobals::allocateServer(char *nodeName, short nodeNameLen, short cpuNum)
{
 ...
 ComDiagsArea *diagsArea = NULL;
  ...
  if (sscpServerClass_ == NULL)
    return NULL;
#ifndef NA_LINUX
  if (nodeName == NULL || (nodeName != NULL && nodeName[0] == '\0'))
  {
    NODENUMBER_TO_NODENAME_ (
    	        ,serverId.nodeName_
   		,MAX_SEGMENT_NAME_LEN
		,&len);
    serverId.nodeName_[len] = '\0';
  }
  else
  {
    str_cpy_all(serverId.nodeName_, nodeName, nodeNameLen);
    serverId.nodeName_[nodeNameLen] = '\0';
  }
#else
...
#ifndef NA_LINUX
  NADELETE(diagsArea, ComDiagsArea, heap_);
#else
...
void SsmpGlobals::work()
{
  ...
  statsGlobals->cleanupDeferredSql();
  ...
  if  ((temp2 > GcInterval) && !doingGC())
  {
    ...
    statsGlobals->checkForDeadProcesses();
    setDoingGC(TRUE);
    short savedPriority, savedStopMode;
  ...
  while (ss != NULL)
  {
        ...
        (ss->getMasterStats()->timeSinceBlocking(minimumAge) > 0))
    {
      ...
      if (parentQid != NULL)
      {
        ...
        {
#if 0
// Don't want to call seapilot while holding a seamphore,
// but keep this disabled code for now, because if enabled
// if can help debug.
          char thisQid[ComSqlId::MAX_QUERY_ID_LEN + 1];
          str_cpy_all(thisQid, ss->getQueryId(), ss->getQueryIdLen());
          thisQid[ss->getQueryIdLen()] = '\0';

          char pQid[ComSqlId::MAX_QUERY_ID_LEN + 1];
          str_cpy_all(pQid, parentQid, parentQidLen);
          pQid[parentQidLen] = '\0';
          ...
          str_sprintf(msg, 
          "getQidFromPid found %s , but will continue searching"
          "for its parent, %s",
           thisQid, pQid);
...
void SscpClientMsgStream::delinkConnection(IpcConnection *conn)
{
  ...
  numOfErrorRequests_++;
  numOfClientRequestsSent_--;
  conn->getOtherEnd().getNodeName().getGuardianNodeNameAsString(nodeName);
