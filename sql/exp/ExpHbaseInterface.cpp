				       ...
				       NAList<Text> &col3ValueList) // output
{
  ...
  {
     case 1:
        columns.insert(col1NameStr);
        col1ValueList.resize(0);
        break;
     case 2:
        columns.insert(col1NameStr);
        columns.insert(col2NameStr);
        col1ValueList.resize(0);
  ...
  while (retcode == HBASE_ACCESS_SUCCESS)
  {
     ...
     {	
        for (int colNo = 0; colNo < numCols; colNo++)
        {
           ...
           if (retcode != HBASE_ACCESS_SUCCESS)
              break;
           NAString columnName((char *)colName, colNameLen);
           BYTE *colVal = NULL;
           colValLen = 0;
