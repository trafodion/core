...
void PCodeCfg::layoutNativeCode(Space* showplanSpace = NULL)
{
 ...
 if ( NExprDbgLvl_ >= VV_I2 ) {
  ...
  //       any chip-specific attributes, we are telling the JIT compiler
  //       to generate machine code for a generic x86-64 chip rather than
  //       letting it default to generating machine code for the specific
  //       chip that we (the SQL Compiler) happens to be running on at
  //       the moment we are compiling.
  //
